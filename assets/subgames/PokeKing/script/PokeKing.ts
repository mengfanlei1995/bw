...
import { PokerKingCmd, Push_GameCmd, Push_Game_BetCmd, Push_Game_EndCmd, Push_Game_StartCmd, Push_Game_TackOutCmd, Push_PokerKingCmd } from "../../../scripts/net/CmdData";
import CmdMgr from "../../../scripts/net/CmdMgr";
import { NotifyBetVO, PKBeginBetVO, PKDoEnterRoomVO, PKSendDrawMsgVO, PokerKingWinDto, decodeNotifyBetVO, decodePKBeginBetVO, decodePKDoEnterRoomVO, decodePKSendDrawMsgVO } from "../../../scripts/net/proto/room";
...
@ccclass
export default class PokeKing extends UIGame {
    ...
    async _enterRoom(): Promise<void> {
        ...
        if (!info) return;
        let data: PKDoEnterRoomVO = decodePKDoEnterRoomVO(info);
    ...
    //初始化房间信息
    _initRoomInfo(info: PKDoEnterRoomVO) {
        this.initRecordHistroy(info.gameInfo.gameResultList);
        this.initRoomInfo(info);
    }

    /**初始化历史记录 */
    private initRecordHistroy(gameResultList: PokerKingWinDto[]) {
    ...
    */
    ...
    _gameStart(data: Uint8Array) {
        let info: PKBeginBetVO = decodePKBeginBetVO(data);
    ...
    _gameEnd(data: Uint8Array) {
        let info: PKSendDrawMsgVO = decodePKSendDrawMsgVO(data);
        console.log('_gameEnd', info);
        let { gameResult } = info;
        let { poker, id } = gameResult
