...
export interface RechargeEventMonthVO {
  has?: boolean;
  remain?: number;
  days?: number;
}

export function encodeRechargeEventMonthVO(message: RechargeEventMonthVO): Uint8Array {
  let bb = popByteBuffer();
  _encodeRechargeEventMonthVO(message, bb);
  return toUint8Array(bb);
}

function _encodeRechargeEventMonthVO(message: RechargeEventMonthVO, bb: ByteBuffer): void {
  // optional bool has = 1;
  let $has = message.has;
  if ($has !== undefined) {
    writeVarint32(bb, 8);
    writeByte(bb, $has ? 1 : 0);
  }

  // optional double remain = 2;
  let $remain = message.remain;
  if ($remain !== undefined) {
    writeVarint32(bb, 17);
    writeDouble(bb, $remain);
  }
...
export function decodeRechargeEventMonthVO(binary: Uint8Array): RechargeEventMonthVO {
  return _decodeRechargeEventMonthVO(wrapByteBuffer(binary));
}

function _decodeRechargeEventMonthVO(bb: ByteBuffer): RechargeEventMonthVO {
  let message: RechargeEventMonthVO = {} as any;
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      // optional bool has = 1;
      case 1: {
        message.has = !!readByte(bb);
        break;
      }

      // optional double remain = 2;
      case 2: {
        message.remain = readDouble(bb);
        break;
      }
