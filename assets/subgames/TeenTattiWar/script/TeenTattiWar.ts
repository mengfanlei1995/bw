...
import { NotifyBetVO, TPBeginBetVO, TPDoEnterRoomVO, TPSendDrawMsgVO, TeenPattiWarWinDto, decodeNotifyBetVO, decodeTPBeginBetVO, decodeTPDoEnterRoomVO, decodeTPSendDrawMsgVO } from "../../../scripts/net/proto/room";
...
@ccclass
export default class TeenTattiWar extends UIGame {
    ...
    async _enterRoom(): Promise<void> {
        ...
        if (!info) return;
        let data: TPDoEnterRoomVO = decodeTPDoEnterRoomVO(info);
    ...
    //初始化房间信息
    _initRoomInfo(info: TPDoEnterRoomVO) {
        this.initRecordHistroy(info.gameInfo.gameResultList);
        this.initRoomInfo(info);
    }

    /**初始化历史记录 */
    private initRecordHistroy(gameResultList: TeenPattiWarWinDto[]) {
     ...
     * @returns 
     */
    openAward(gameResult: TeenPattiWarWinDto) {
    ...
    */
    ...
    _gameStart(data: Uint8Array) {
        let info: TPBeginBetVO = decodeTPBeginBetVO(data);
    ...
    _gameEnd(data: Uint8Array) {
        let info: TPSendDrawMsgVO = decodeTPSendDrawMsgVO(data);
